(function() {
  var fn = function() {
    
    (function(root) {
      function now() {
        return new Date();
      }
    
      var force = false;
    
      if (typeof root._bokeh_onload_callbacks === "undefined" || force === true) {
        root._bokeh_onload_callbacks = [];
        root._bokeh_is_loading = undefined;
      }
    
      
      
    
      var element = document.getElementById("7f34cd87-ae81-4789-9ac4-99a76b6b193c");
        if (element == null) {
          console.warn("Bokeh: autoload.js configured with elementid '7f34cd87-ae81-4789-9ac4-99a76b6b193c' but no matching script tag was found.")
        }
      
    
      function run_callbacks() {
        try {
          root._bokeh_onload_callbacks.forEach(function(callback) {
            if (callback != null)
              callback();
          });
        } finally {
          delete root._bokeh_onload_callbacks
        }
        console.debug("Bokeh: all callbacks have finished");
      }
    
      function load_libs(css_urls, js_urls, callback) {
        if (css_urls == null) css_urls = [];
        if (js_urls == null) js_urls = [];
    
        root._bokeh_onload_callbacks.push(callback);
        if (root._bokeh_is_loading > 0) {
          console.debug("Bokeh: BokehJS is being loaded, scheduling callback at", now());
          return null;
        }
        if (js_urls == null || js_urls.length === 0) {
          run_callbacks();
          return null;
        }
        console.debug("Bokeh: BokehJS not loaded, scheduling load and callback at", now());
        root._bokeh_is_loading = css_urls.length + js_urls.length;
    
        function on_load() {
          root._bokeh_is_loading--;
          if (root._bokeh_is_loading === 0) {
            console.debug("Bokeh: all BokehJS libraries/stylesheets loaded");
            run_callbacks()
          }
        }
    
        function on_error(url) {
          console.error("failed to load " + url);
        }
    
        for (let i = 0; i < css_urls.length; i++) {
          const url = css_urls[i];
          const element = document.createElement("link");
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.rel = "stylesheet";
          element.type = "text/css";
          element.href = url;
          console.debug("Bokeh: injecting link tag for BokehJS stylesheet: ", url);
          document.body.appendChild(element);
        }
    
        const hashes = {"https://cdn.bokeh.org/bokeh/release/bokeh-2.3.0.min.js": "HjagQp6T0/7bxYTAXbLotF1MLAGWmhkY5siA1Gc/pcEgvgRPtMsRn0gQtMwGKiw1", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.0.min.js": "ZEPPTjL+mdyqgIq+/pl9KTwzji8Kow2NnI3zWY8+sFinWP/SYJ80BnfeJsa45iYj", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.0.min.js": "exLqv2ACDRIaV7ZK1iL8aGzGYQvKVuT3U2CT7FsQREBxRah6JrkVCoFy0koY1YqV"};
    
        for (let i = 0; i < js_urls.length; i++) {
          const url = js_urls[i];
          const element = document.createElement('script');
          element.onload = on_load;
          element.onerror = on_error.bind(null, url);
          element.async = false;
          element.src = url;
          if (url in hashes) {
            element.crossOrigin = "anonymous";
            element.integrity = "sha384-" + hashes[url];
          }
          console.debug("Bokeh: injecting script tag for BokehJS library: ", url);
          document.head.appendChild(element);
        }
      };
    
      function inject_raw_css(css) {
        const element = document.createElement("style");
        element.appendChild(document.createTextNode(css));
        document.body.appendChild(element);
      }
    
      
      var js_urls = ["https://cdn.bokeh.org/bokeh/release/bokeh-2.3.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-widgets-2.3.0.min.js", "https://cdn.bokeh.org/bokeh/release/bokeh-tables-2.3.0.min.js"];
      var css_urls = [];
      
    
      var inline_js = [
        function(Bokeh) {
          Bokeh.set_log_level("info");
        },
        
        function(Bokeh) {
          (function() {
            var fn = function() {
              Bokeh.safely(function() {
                (function(root) {
                  function embed_document(root) {
                    
                  var docs_json = '{"19335e7d-bcda-483b-a575-8b995c414ddd":{"defs":[{"extends":null,"module":null,"name":"DataModel","overrides":[],"properties":[]}],"roots":{"references":[{"attributes":{},"id":"14728","type":"UndoTool"},{"attributes":{"children":[{"id":"14778"},{"id":"14776"}]},"id":"14779","type":"Column"},{"attributes":{"data":{},"selected":{"id":"14773"},"selection_policy":{"id":"14774"}},"id":"14753","type":"ColumnDataSource"},{"attributes":{"formatter":{"id":"14766"},"major_label_policy":{"id":"14765"},"ticker":{"id":"14716"}},"id":"14715","type":"LinearAxis"},{"attributes":{},"id":"14762","type":"AllLabels"},{"attributes":{"bottom":1.4596776592684024,"fill_alpha":0.1,"fill_color":"#2a2eec","top":0.6850827603275129},"id":"14747","type":"BoxAnnotation"},{"attributes":{},"id":"14765","type":"AllLabels"},{"attributes":{"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"14743","type":"Line"},{"attributes":{"source":{"id":"14748"}},"id":"14752","type":"CDSView"},{"attributes":{"data_source":{"id":"14742"},"glyph":{"id":"14743"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14744"},"view":{"id":"14746"}},"id":"14745","type":"GlyphRenderer"},{"attributes":{},"id":"14770","type":"UnionRenderers"},{"attributes":{},"id":"14774","type":"UnionRenderers"},{"attributes":{},"id":"14716","type":"BasicTicker"},{"attributes":{"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"14754","type":"Line"},{"attributes":{"toolbar":{"id":"14777"},"toolbar_location":"above"},"id":"14778","type":"ToolbarBox"},{"attributes":{},"id":"14729","type":"SaveTool"},{"attributes":{},"id":"14726","type":"WheelZoomTool"},{"attributes":{},"id":"14771","type":"Selection"},{"attributes":{"fill_alpha":0.5,"fill_color":"lightgrey","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"syncable":false,"xs_units":"screen","ys_units":"screen"},"id":"14732","type":"PolyAnnotation"},{"attributes":{"axis":{"id":"14719"},"dimension":1,"ticker":null},"id":"14722","type":"Grid"},{"attributes":{"line_alpha":0.1,"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"14750","type":"Line"},{"attributes":{},"id":"14766","type":"BasicTickFormatter"},{"attributes":{},"id":"14711","type":"LinearScale"},{"attributes":{"source":{"id":"14753"}},"id":"14757","type":"CDSView"},{"attributes":{"data_source":{"id":"14753"},"glyph":{"id":"14754"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14755"},"view":{"id":"14757"}},"id":"14756","type":"GlyphRenderer"},{"attributes":{},"id":"14773","type":"Selection"},{"attributes":{"line_alpha":0.1,"line_color":"#2a2eec","x":{"field":"x"},"y":{"field":"y"}},"id":"14744","type":"Line"},{"attributes":{"data_source":{"id":"14748"},"glyph":{"id":"14749"},"hover_glyph":null,"muted_glyph":null,"nonselection_glyph":{"id":"14750"},"view":{"id":"14752"}},"id":"14751","type":"GlyphRenderer"},{"attributes":{},"id":"14772","type":"UnionRenderers"},{"attributes":{"data":{"x":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]},"y":{"__ndarray__":"","dtype":"float64","order":"little","shape":[512]}},"selected":{"id":"14769"},"selection_policy":{"id":"14770"}},"id":"14742","type":"ColumnDataSource"},{"attributes":{},"id":"14707","type":"DataRange1d"},{"attributes":{"text":"y / y","text_font_size":"15pt"},"id":"14758","type":"Title"},{"attributes":{"overlay":{"id":"14732"}},"id":"14727","type":"LassoSelectTool"},{"attributes":{"below":[{"id":"14715"}],"center":[{"id":"14718"},{"id":"14722"},{"id":"14747"}],"height":500,"left":[{"id":"14719"}],"output_backend":"webgl","renderers":[{"id":"14745"},{"id":"14751"},{"id":"14756"}],"title":{"id":"14758"},"toolbar":{"id":"14733"},"toolbar_location":null,"width":500,"x_range":{"id":"14707"},"x_scale":{"id":"14711"},"y_range":{"id":"14709"},"y_scale":{"id":"14713"}},"id":"14706","subtype":"Figure","type":"Plot"},{"attributes":{"children":[[{"id":"14706"},0,0]]},"id":"14776","type":"GridBox"},{"attributes":{"active_multi":null,"tools":[{"id":"14723"},{"id":"14724"},{"id":"14725"},{"id":"14726"},{"id":"14727"},{"id":"14728"},{"id":"14729"},{"id":"14730"}]},"id":"14733","type":"Toolbar"},{"attributes":{"callback":null},"id":"14730","type":"HoverTool"},{"attributes":{"formatter":{"id":"14763"},"major_label_policy":{"id":"14762"},"ticker":{"id":"14720"}},"id":"14719","type":"LinearAxis"},{"attributes":{"line_alpha":0.1,"line_color":"#1f77b4","x":{"value":0},"y":{"value":0}},"id":"14755","type":"Line"},{"attributes":{"toolbars":[{"id":"14733"}],"tools":[{"id":"14723"},{"id":"14724"},{"id":"14725"},{"id":"14726"},{"id":"14727"},{"id":"14728"},{"id":"14729"},{"id":"14730"}]},"id":"14777","type":"ProxyToolbar"},{"attributes":{"line_color":"white","x":{"field":"x"},"y":{"field":"y"}},"id":"14749","type":"Line"},{"attributes":{"bottom_units":"screen","fill_alpha":0.5,"fill_color":"lightgrey","left_units":"screen","level":"overlay","line_alpha":1.0,"line_color":"black","line_dash":[4,4],"line_width":2,"right_units":"screen","syncable":false,"top_units":"screen"},"id":"14731","type":"BoxAnnotation"},{"attributes":{},"id":"14713","type":"LinearScale"},{"attributes":{},"id":"14720","type":"BasicTicker"},{"attributes":{},"id":"14723","type":"ResetTool"},{"attributes":{},"id":"14724","type":"PanTool"},{"attributes":{"axis":{"id":"14715"},"ticker":null},"id":"14718","type":"Grid"},{"attributes":{},"id":"14769","type":"Selection"},{"attributes":{},"id":"14709","type":"DataRange1d"},{"attributes":{},"id":"14763","type":"BasicTickFormatter"},{"attributes":{"overlay":{"id":"14731"}},"id":"14725","type":"BoxZoomTool"},{"attributes":{"data":{"x":[0,1],"y":[1,1]},"selected":{"id":"14771"},"selection_policy":{"id":"14772"}},"id":"14748","type":"ColumnDataSource"},{"attributes":{"source":{"id":"14742"}},"id":"14746","type":"CDSView"}],"root_ids":["14779"]},"title":"Bokeh Application","version":"2.3.0"}}';
                  var render_items = [{"docid":"19335e7d-bcda-483b-a575-8b995c414ddd","root_ids":["14779"],"roots":{"14779":"7f34cd87-ae81-4789-9ac4-99a76b6b193c"}}];
                  root.Bokeh.embed.embed_items(docs_json, render_items);
                
                  }
                  if (root.Bokeh !== undefined) {
                    embed_document(root);
                  } else {
                    var attempts = 0;
                    var timer = setInterval(function(root) {
                      if (root.Bokeh !== undefined) {
                        clearInterval(timer);
                        embed_document(root);
                      } else {
                        attempts++;
                        if (attempts > 100) {
                          clearInterval(timer);
                          console.log("Bokeh: ERROR: Unable to run BokehJS code because BokehJS library is missing");
                        }
                      }
                    }, 10, root)
                  }
                })(window);
              });
            };
            if (document.readyState != "loading") fn();
            else document.addEventListener("DOMContentLoaded", fn);
          })();
        },
        function(Bokeh) {
        
        
        }
      ];
    
      function run_inline_js() {
        
        for (var i = 0; i < inline_js.length; i++) {
          inline_js[i].call(root, root.Bokeh);
        }
        
      }
    
      if (root._bokeh_is_loading === 0) {
        console.debug("Bokeh: BokehJS loaded, going straight to plotting");
        run_inline_js();
      } else {
        load_libs(css_urls, js_urls, function() {
          console.debug("Bokeh: BokehJS plotting callback run at", now());
          run_inline_js();
        });
      }
    }(window));
  };
  if (document.readyState != "loading") fn();
  else document.addEventListener("DOMContentLoaded", fn);
})();